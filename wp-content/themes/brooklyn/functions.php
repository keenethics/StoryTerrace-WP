        ...
        */    
        ...
        if( preg_match("/chrome/",strtolower($_SERVER['HTTP_USER_AGENT'])) && !$detect->isMobile() ) {
            ...
            if(strpos($user_agent, "Win") !== FALSE ) {
                // THIS DOESN't SEEM TO WORK WITH CHROME ANYMORE
				// wp_enqueue_script( 'ut-smoothscroll',    THEME_WEB_ROOT . '/js/SmoothScroll.min.js', array('jquery'), '0.9.9' , true );     
            }
            
        }
        
        /* retina images *///TODO MAKE THIS WORK WITH RETINA IMAGES - ourwriters page has issue with the tabs thing
...
if ( !in_array($posted_data["your-email"], array('testingemail@domain.com','anotheremail@domain.com','blacklistedemail@domain.com'), true ) ) { 
  ...
  $data = json_encode(array(
      ...
      "phone" => "{$posted_data["your-phone-number"]}",
      "message" => "{$posted_data["your-message"]}",
      "id" => "{$posted_data["submit_time"]}",
      "Submitted" => "$timestamp",
      "From URL" => "$url",
      "From Referer" => "{$posted_data["referer-page"]}",
      "Server Referer" => "{$_SERVER['HTTP_REFERER']}",
      "From IP" => "$remote_ip",
      "Page Title" => "$title"
      ));
 
   // Finally send the data to Zapier or your other webhook endpoint
        $ch = curl_init("https://hooks.zapier.com/hooks/catch/974347/9qt15o/"); // replace with your Zapier webook
...
/**
 * WPML Support for https://wpml.org/forums/topic/manually-set-prices-in-other-currencies/
 */
function wpmlsupp_1867_add_product_variations( $icl_post_types ) {
    $icl_post_types['product_variation'] = get_post_type_object( 'product_variation' );
  
    return $icl_post_types;
}
add_filter( 'get_translatable_documents', 'wpmlsupp_1867_add_product_variations' );
